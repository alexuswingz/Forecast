"""
Backfill daily_product_metrics table with historical data
This aggregates data from order_items, child_traffic_metrics, and ad_product_performance
"""
import sys
...
def backfill_metrics():
    ...
    print("=" * 80)
    print("Backfilling Daily Product Metrics")
    print("=" * 80)
    ...
    with engine.connect() as conn:
        ...
        # Insert aggregated data with progress
        insert_sql = """
        ...
        all_dates AS (
            SELECT DISTINCT asin, date FROM sales_daily
            UNION
            SELECT DISTINCT asin, date FROM traffic_daily
            UNION
            SELECT DISTINCT asin, date FROM ads_daily
        )
        SELECT 
            d.asin,
            d.date,
        ...
        print("=" * 80)
        for row in result:
            print(f"  {row[0]} | {row[1]} | Units: {row[2]:,} | Sales: ${row[3]:,.2f} | Sessions: {row[4]:,} | Ad Spend: ${row[5]:,.2f}")
        
        print("\n" + "=" * 80)
        print("Next step: Deploy updated Lambda function")
        print("=" * 80)
        print()
